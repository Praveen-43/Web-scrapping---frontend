...
export function AmazonDash() {
  ...
  useEffect(() => {
    const getDetails = async () => {
      try {
        const res = await fetch("https://praveen-43-web-scrapping-backend.onrender.com", {
          method: "GET",
          headers: {
  ...
  const editPrice = async () => {
    try {
      const response = await fetch(`https://praveen-43-web-scrapping-backend.onrender.com/${id}`, {
        method: "PUT",
        body: JSON.stringify({
          buyPrice: price,
        }),
  ...
  const handleDelete = async (item) => {
    try {
      ...
      const res = await fetch(`https://praveen-43-web-scrapping-backend.onrender.com/${idx}`, {
        method: "DELETE",
        headers: {
          "x-auth-token": localStorage.getItem("token"),
        },
